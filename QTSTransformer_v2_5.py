...
# --------------------------------------------------------------------------------
# Helper Functions for LCU and QSVT Classical Simulation
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
# Quantum Time-Series Transformer v2.5 — Classical QSVT/LCU Simulation
#
# v2.5 improvements over v1 (QTSTransformer.py):
#   1. Sinusoidal Positional Encoding (Vaswani et al., 2017)
#   2. 2π angle scaling: sigmoid * 2π for full rotation range [0, 2π]
#
# Architecture:
#   Input (batch, C, T) → permute → (batch, T, C)
#     → + Sinusoidal PE (Vaswani et al., 2017)
#     → Linear(C, n_rots) + Sigmoid * 2π → [0, 2π]
#     → Classical LCU+QSVT simulation (Khatri Section 4.1)
#     → QFF sim14 → measure PauliX/Y/Z → output
# --------------------------------------------------------------------------------
...
class QuantumTSTransformer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # x: (batch, feature_dim, n_timesteps) → (batch, n_timesteps, feature_dim)
        x = x.permute(0, 2, 1)
        x = x + self.pe[:, :x.size(1)]                    # positional encoding

        x = self.feature_projection(self.dropout(x))
        timestep_params = self.rot_sigm(x) * (2 * math.pi) # [0, 2π] angle range
